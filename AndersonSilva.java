...
public class AndersonSilva extends AdvancedRobot {
     ...
     */
		...
		double anguloAbsolutoInimigo = Math.toRadians(getHeading() + enemyBearing);
		double enemyX = getX() + Math.sin(anguloAbsolutoInimigo) * enemyDistance;
		double enemyY = getY() + Math.cos(anguloAbsolutoInimigo) * enemyDistance;
		        
        // Posição futura do inimigo (predição linear)
		double futuroX = enemyX + Math.sin(Math.toRadians(enemyHeading)) * enemyVelocity * tempoParaImpacto;
		double futuroY = enemyY + Math.cos(Math.toRadians(enemyHeading)) * enemyVelocity * tempoParaImpacto;
        ...
        // Calcula o ângulo para a posição futura
      	double anguloParaFuturo = Math.toDegrees(Math.atan2(futuroX - getX(), futuroY - getY()));
